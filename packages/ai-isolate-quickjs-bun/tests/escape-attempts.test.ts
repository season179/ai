...
// quickjs-bun is built on bun:ffi, so this suite only runs under Bun.
// Run locally with: bun test ./tests
describe.skipIf(typeof Bun === 'undefined')(
  'QuickJS Bun isolate — sandbox escape attempts',
  () => {
    ...
    it('does not expose `Bun`', async () => {
      const res = await runInIsolate('return typeof Bun')
      expect(res.success).toBe(true)
      expect(res.value).toBe('undefined')
    })

    it('does not expose timers or the host event loop', async () => {
      const res = await runInIsolate(
        'return [typeof setTimeout, typeof setInterval, typeof queueMicrotask].join(",")',
      )
      expect(res.success).toBe(true)
      expect(res.value).toBe('undefined,undefined,undefined')
    })

    it('does not include the QuickJS std and os modules', async () => {
      const res = await runInIsolate(`
      try {
